...
import { ApiResponse, TVProps } from "../types/app";
...
export const fetchTvsByPage = async (
  ...
  page: number
): Promise<ApiResponse<TVProps>> => {
  const res = await axios.get<ApiResponse<TVProps>>(`${API_URL}/tv/${type}?language=ko-KR&page=${page}`, API_OPTIONS);
...
export const fetchKoreanTvs = async (
  type: "popular" | "top_rated" | "on_the_air",
  setState: React.Dispatch<React.SetStateAction<TVProps[]>>,
